...
/**
 * Pour le futur :
 * - Calcul intersection : pb si a = 0 (div par zéro)
 * - Faire un Système de création de chemin de ronde 
 * - Faire les Prédicates et les comparators usuels
 * - Pb algo du projeté orthogonal : (cas limitte) :
 *   vérifier que le monstre ne sort pas de la carte avant d'entrer dans le puits de gravité.
 * 
 *  Idées :
 *  - Ne pas stocker des List de Pion mais des Stream directement
 *    comme ça une liste des défenseurs c'est uniquement le Stream heros auquel on ajoute une filter
 *    pour faire un nouveau Stream
  * 
 **/
class Player {
    ...
    public void joue(InputStream inputStream) {
	    ...
	    while (true) {
	        ...
	        for (int i = 0; i < entityCount; i++) {
	            ...
	            pion.isControlled = in.nextInt(); // Ignore for this league; Equals 1 when this entity is under a control spell
	            ...
	            pion.threatFor = in.nextInt(); // Given this monster's trajectory, is it a threat to 1=your base, 2=your opponent's base, 0=neither
	        ...
	        heros.forEach(h -> {
	        	if (h.action == 1) {
	        		...
	        		System.out.println("SPELL CONTROL "+ h.targetId + " " + h.targetx + " " + h.targety+ " " + h.comment);
	        	} else if (h.action == 4) {
    ...
    private void init() {
		if (base.x == 0) {
       	...
       	for (int i = 0; i < strategies.length; i++) strategies[i] = STRATEGIE_DEF;
	...
	private void evolutionStrategie(List<Pion> heros, List<Pion> advs, Map<Integer, Pion> mstrs) {
		advEnvoieControls += heros.stream()
				.filter(p -> p.isControlled == 1)
				.count();
		...
		for (int i = 0; i < strategies.length; i++) heros.get(i).tactiq = strategies[i];
    ...
    private void tactiqueDefensive(List<Pion> heros, Map<Integer, Pion> mstrs) {
    	...
    	List<Pion> attackers = mstrs.values().stream()
        	.filter(m -> !m.affected)
    		.filter(m -> m.nearBase == 1 && m.threatFor == 1)
    		.sorted((p1, p2) -> p1.distance(base) < p2.distance(base) ? -1 : 1)
...
//    			return;
	...
	private boolean spellWindDefensif(List<Pion> heros, Pion mstr, Map<Integer, Pion> mstrs) {
    	...
    	Optional<Pion> hSelected = heros.stream()
			.filter(h -> !h.affected && h.isControlled != 1)
	...
	private void tactiqueAntiButeur(List<Pion> heros, List<Pion> advs, Map<Integer, Pion> mstrs) {
		...
		if (mana[0] < 20 || mana[1] < 10) return;

		double DISTANCE_TO_BASE = 7_000D;
		double MIN_DISTANCE_POUR_SPELL = 1_280D;
		
		// Recherche d'un buteur
    	Optional<Pion> butteurAdv = advs.stream()
        		.filter(p -> p.shieldLife == 0)
    	...
    	if (butteurAdv.isPresent()) {
    		...
    		List<Pion> mstrsProchesAdv = mstrs.values().stream()
    			.filter(m -> m.distance(adv) < 2_200D)
    			.collect(Collectors.toList());
    		...
    		if (mstrsProchesAdv.size() > 0) {
				...
				heros.stream()
    				...
    				.filter(h -> {
    					log("Hero", h);
    					List<Pion> mstrsProchesHero = mstrs.values().stream()
		        			.filter(m -> m.distance(h) < 1_280D)
		        			.collect(Collectors.toList());
    				...
    				.reduce((h1, h2) -> h1.distance(adv) < h2.distance(adv) ? h1 : h2)
    				.ifPresent(h -> {
    					log("select h:", h, ", h.distance(adv): ", h.distance(adv));
    					if (h.distance(adv) < MIN_DISTANCE_POUR_SPELL
    							&& mana[0] >= 10) {
    			    		log(" > hero: "+h.id);
    			    		h.affected = true;
    			    		h.action = 2;
	...
	private void tactiqueAutoShield(List<Pion> heros, List<Pion> advs) {
		...
		// Recherche d'un buteur
    	Optional<Pion> buteurAdvPresent = advs.stream()
        		.filter(p -> p.distance(base) < DISTANCE_TO_BASE)
    			.reduce((p1, p2) -> p1);
	...
	private void tactiqueButeur(List<Pion> heros, List<Pion> advs, Map<Integer, Pion> mstrs) {
		...
		if (mana[0] > 30 && DO_PROTECTION) {
	    	...
	    	List<Pion> mProches = mstrs.values().stream()
	    			.filter(m -> m.distance(buteur) < 2_200D)
	    			.filter(m -> m.shieldLife == 0)
	    			.filter(m -> m.distance(baseAdv) < 5_000D)
	    			//.filter(m -> m.willFallIntoBase(baseAdv))
	    			.filter(m -> m.health > 5)
	    			.sorted((p1, p2) -> p1.health > p2.health ? -1 : 1)
	         		.collect(Collectors.toList());
		...
		if (mana[0] >= 10
				&& NB_MIN_MONSTRE_POUR_WIND > 0
				&& buteur.distance(baseAdv) < DISTANCE_BASE_ADV_POUR_WIND) {
			// Compter le nb de monstres que l'on peut envoyer par un spell wind
	    	List<Pion> mProches = mstrs.values().stream()
	    			.filter(m -> m.distance(buteur) < 1_280D)
...
/*		
    	Optional<Pion> adv = advs.stream()
         		.reduce((p1, p2) -> p1.distance(base) < p2.distance(base) ? p1 : p2);
    	if (adv.isPresent())  maxDistance = adv.get().distance(base) + 3_000D;
    	
		log("maxDistance: "+ maxDistance);
		Double max = new Double(maxDistance);
*/		
    	List<Pion> freeHeros = heros.stream()
    			.filter(m -> !m.affected)
    	...
    	for (Pion h : freeHeros) {
	    	...
	    	Optional<Pion> mProches = mstrs.values().stream()
	    			.peek(Player::log)
	        		.filter(m -> !m.affected)
	        		.filter(m -> !m.willFallIntoBase(baseAdv))
	    			.filter(m -> m.distance(h) < MAX_DISTANCE_DU_MONSTRE )
	    			.filter(m -> m.distance(base) < maxDistance )
	         		.reduce((m1, m2) -> {
	         			if (h.getInterception(m1).round < h.getInterception(m2).round) return m1;
	         			else return (m1.distance(base) < m2.distance(base) ? m1 : m2);});
	...
	private void affectHerosSurMonstres(List<Pion> heros, List<Pion> attackers, int nbDef) {
    	...
    	for (int n = 0; n < nbDef; n++) {
	    	...
	    	.forEach(m -> {
		    	...
		    	Optional<Pion> heroSelected = heros.stream()
		    			.peek(Player::log)
		        		.filter(h -> !h.affected)
		    	...
		    	if (heroSelected.isPresent()) {
		    		...
		    		if (m.shieldLife > 0) m.affected = false;
		    	}
	    	});
    	}
	}
	...
	private void affectHeroSurMonstre(Pion h, Pion m, Collection<Pion> mstrs) {
		...
		if (doAttaqueDirecte) {
			...
			if (mstrs != null) {
		    	...
		    	List<Pion> mProches = mstrs.stream()
		    			.filter(p -> p.id != m.id)
		    			.sorted((p1, p2) -> p1.nextCoords().distance(m.nextCoords()) < p2.nextCoords().distance(m.nextCoords()) ? -1 : 1)
		    			.collect(Collectors.toList());
				if (mProches.size() > 0) {
	...
	public class Coords {
		...
		/**
		 * Pour cette Coords, retourne un point opposé aux Coords base à une certaine distance
	...
	public class Pion extends Coords {
		...
		public String dump() {
			return id + " " + type + " " + x + " " + y + " " + shieldLife
					+ " " + isControlled + " " + health + " " + vx + " " + vy
					+ " " + nearBase + " " + threatFor;
		}
